...
void SimulatedEnclaveMemory::init(KeystoneDevice* dev, vaddr_t phys_addr, size_t min_pages)
{
  pDevice = dev;
...
void PhysicalEnclaveMemory::init(KeystoneDevice* dev, vaddr_t phys_addr, size_t min_pages)
{
...
vaddr_t PhysicalEnclaveMemory::allocUTM(int eid, size_t size) {
  vaddr_t ret = pDevice->initUTM(eid, size);
  utmFreeList = ret;
  return ret;
}
...
vaddr_t SimulatedEnclaveMemory::allocUTM(int eid, size_t size) {
...
bool Memory::allocPage(vaddr_t va, vaddr_t src, unsigned int mode)
{
...
{
	unsigned long free_ppn = ppn(epmFreeList);
...
Memory::__ept_walk_internal(vaddr_t addr, int create)
{
	...
	for (i = (VA_BITS - RISCV_PGSHIFT) / RISCV_PGLEVEL_BITS - 1; i > 0; i--) {
      ...
      return create ? __ept_continue_walk_create(addr, &t[idx]) : 0;
			...
			t = (pte_t *) ReadMem((vaddr_t) pte_ppn(t[idx]) << RISCV_PGSHIFT, PAGE_SIZE);
	}
...
{
  pte_t* pte = (pte_t *) __ept_walk(addr);
  if(pte)
